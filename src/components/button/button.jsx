...
const Button = function({type,className,onClick,disabled,icon,src,alt,size = "sm",children}) {
    ...
    return (
        <Fragment>
            <button
                type={type||"button"}
                className={`button-container ${size === 'sm' ? "button-sm" : "button-md"}  ${className}`}
                onClick={onClick}
                disabled={disabled}>
                <div className={`button-content ${icon === 'only' ? "margin-0" : "margin-6"}`}>
                    {content}
                </div>
            </button>
        </Fragment>
...
export const PrimaryButton = function ({type,className,onClick,disabled,icon,src,alt,size,children}){
...
export const SecondaryButton = function ({type,className,onClick,disabled,icon,src,alt,size,children}){
